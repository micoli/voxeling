...
export default Class Server {
	function constructor(config, chunkStore, serverSettings, clientSettings) {
	...
	*/
	...
	function connectClient (wseSocket) {
		var self = this;
	...
	function requestNearbyChunks (position) {
		var self = this;
	...
	function sendChunk (chunk) {
		...
		for (i = 0; i < clientIds.length; i++) {
			...
			if (clientId in self.clients) {
				client = self.clients[clientId];
				if (client.connected) {
					encoded = this.encodedChunkCache.get(chunkId);
					if (!encoded) {
						if (debug) {
	...
	function isChunkInBounds (chunkID) {
		var self = this;
