...
var addPoints = function(basePosition: any, face: any, textureValue: any, x:number, y:number, z:number) {
	if (!textureValue) {
		...
		for (var y = 0; y < chunkSize; y++) {
			for (var x = 0; x < chunkSize; x++, index++) {
				...
				if (z === outside) {
					...
					oppositeFaceIndex = Coordinator.coordinatesToVoxelIndex(x, y, z + 1);
				...
				if (x === 0) {
					...
					oppositeFaceIndex = Coordinator.coordinatesToVoxelIndex(x - 1, y, z);
					if (voxels[oppositeFaceIndex] === 0) {
						// not blocked
						// Add
						addPoints(basePosition, 'left', textureValue, x, y, z);
				...
				if (y === outside) {
					...
					oppositeFaceIndex = Coordinator.coordinatesToVoxelIndex(x, y + 1, z);
					if (voxels[oppositeFaceIndex] === 0) {
						// not blocked
						// Add
						addPoints(basePosition, 'top', textureValue, x, y + 1, z);
...
module.exports = {
	config: function(cs: any, texturesByValue: any, coordinatorHandle: any) {
		chunkSize = cs;
