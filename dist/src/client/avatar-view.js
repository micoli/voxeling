...
class avatarView {
    ...
    ginit() {
        ...
        createSkinTexture(this.game.shell.gl, textureURI + textureFile, textureFile, 'image/png', function (err, texture) {
            self.skin = texture;
        });
        self.mesh = createSkinMesh(this.game.shell.gl);
        self.meshShader = glShader(this.game.shell.gl, glslify('./avatar.vert'), glslify('./avatar.frag'));
    }
    updatePerspective() {
