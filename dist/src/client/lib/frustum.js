...
class Frustum {
    constructor(verticalFieldOfView, ratio, nearDistance, farDistance) {
        ...
        this.coordinates = [
            // near bottom left
            vec3.fromValues(-this.nearWidth, -this.nearHeight, -nearDistance),
            // near bottom right
            vec3.fromValues(this.nearWidth, -this.nearHeight, -nearDistance),
            // near top right
            vec3.fromValues(this.nearWidth, this.nearHeight, -nearDistance),
            ...
            vec3.fromValues(-this.nearWidth, this.nearHeight, -nearDistance),
            // far bottom left
            vec3.fromValues(-this.farWidth, -this.farHeight, -farDistance),
            // far bottom right
            vec3.fromValues(this.farWidth, -this.farHeight, -farDistance),
            // far top right
            vec3.fromValues(this.farWidth, this.farHeight, -farDistance),
    ...
    overlap(a, b0, b1) {
        return ((a[0] <= b0 && b0 <= a[1])
            ||
                (a[0] <= b1 && b1 <= a[1]));
    }
    separatingAxisTheorum(chunkProjections, points) {
        // AABB axes
        var aabbAxes = [
            [1, 0, 0],
            [0, 1, 0],
        ...
        for (var i = 0; i < aabbAxes.length; i++) {
            ...
            var projection = this.project(axis, points);
            // do the projections overlap?
            if (!this.overlap(projection, chunkProjections[ii], chunkProjections[ii + 1])) {
    ...
    visible(chunkID) {
        ...
        if (this.separatingAxisTheorum(projection, this.points)) {
            return true;
        }
        return false;
    }
