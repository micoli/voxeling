...
var cache = new lru_1.LRU(200);
var worldId = 1;
class MysqlChunkStore extends chunk_store_1.ChunkStore {
    ...
    gotChunkChanges(chunks) {
        ...
        for (var chunkID in chunks) {
            ...
            voxel_stats_1.VoxelStats.count('blocks.changed', chunks[chunkID].length / 2);
        }
    }
    applyChanges() {
        var self = this;
    ...
    saveVoxels(chunkID, chunk) {
        ...
        zlib.gzip(new Buffer(chunk.voxels), function (error, buffer) {
            ...
            self.mysqlPool.query('REPLACE INTO chunk SET ?', {
                x: chunk.position[0],
                y: chunk.position[1],
                z: chunk.position[2],
                voxels: buffer,
...
exports.MysqlChunkStore = MysqlChunkStore;
