...
var debug = false;
class ServerRegionsGenerator extends generator_1.Generator {
    constructor(chunkSize, folder) {
        super(chunkSize);
        this.chunkFolder = folder;
    }
    fillChunkVoxels(chunk, fn, width) {
...
exports.ServerRegionsGenerator = ServerRegionsGenerator;
function getRandomInt(min, max) {
    return Math.floor(Math.random() * (max - min)) + min;
}
function perlin(seed, floor, ceiling, divisor) {
    floor = floor || 0;
    ceiling = ceiling || 20;
    // minecraft's limit
    divisor = divisor || 50;
