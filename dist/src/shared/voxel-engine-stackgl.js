...
class Game extends events_1.EventEmitter {
    constructor(opts) {
        ...
        this.arrayType = {};
        this.gravity = [0, -0.0000036, 0];
        this.friction = 0.3;
        this.epilson = 1e-8;
        this.terminalVelocity = [0.9, 0.1, 0.9];
        this.defaultButtons = {
            'W': 'forward',
            'A': 'left',
            'S': 'backward',
        ...
        this.isClient = Boolean((typeof opts.isClient !== 'undefined') ? opts.isClient : process.hasOwnProperty('browser'));
        ...
        console.log('this.chunkSize', this.chunkSize, 'this.chunkPad', this.chunkPad);
        ...
        obsolete(this, 'view');
        // used to be a three.js PerspectiveCamera set by voxel-view; see also basic-camera but
        ...
        this.collideVoxels = collisions(this.getBlock.bind(this), 1, [Infinity, Infinity, Infinity], [-Infinity, -Infinity, -Infinity]);
        ...
        if (this.isClient) {
            if ('materials' in opts) {
                throw new Error('opts.materials replaced with voxel-registry registerBlock()'); // TODO: bridge?
            }
        }
        ...
        for (let name in this.isClient ? BUILTIN_PLUGIN_OPTS : SERVER_BUILTIN_PLUGIN_OPTS) {
            pluginOpts[name] = pluginOpts[name] || (this.isClient ? BUILTIN_PLUGIN_OPTS : SERVER_BUILTIN_PLUGIN_OPTS)[name];
        }
    ...
    addItem(item) {
        if (!item.tick) {
            var newItem = physical(item.mesh, this.potentialCollisionSet(), [item.size, item.size, item.size]);
            if (item.velocity) {
    ...
    raycast(start, direction, maxDistance, epilson) {
    ...
    // Calculate fraction of each voxel type in chunk, for debuggig
    showChunk(chunk, optionalPosition) {
        if (!this.isClient) {
            return;
        }
        if (optionalPosition) {
    ...
    setInterval() {
        return tic.interval.bind(tic);
    }
    setTimeout() {
        return tic.timeout.bind(tic);
    }
    tick(delta) {
        for (var i = 0, len = this.items.length; i < len; ++i) {
        ...
        if (!this.controls) {
            return;
        } // this.controls removed; still load chunks
